...
The initial values of model components and the parameters defining the relationships among model components can be changed in order to evaluate how the model behaves under different conditions. If left unspecified, simPanic will use default initial values and parameters. 

To specify initial values or parameters of interest, first create a list assigning the chosen value for the relevant component or parameter. Here, we specify an initial value of 1 for arousal schema (S) and we specify a value of .55 for the parameter r_A, a rate parameter constraining the rate of change for arousal A:
...
Then use that list to define initial values (initial = ) and parameters (parameters = ) in the simPanic function and run simPanic: 
...
To confirm we successfully defined the values of interest for this simulation, we check the input initial values and parameters:
...
### Visualize Simulation Results for Key Model Components
We can now visualize the simulation results with these new intial values and parameter settings. 
...
For example, we can specify that the perturbation should occur at minute 10 and that the perturbation should move arousal to A=0.60. 
...
Then we use this list to define initial values (initial = ) and parameters (parameters = ) in the simPanic function: 
...
Note that the response to these perturbations are affected by stochastic variation in arousal. As a result, the response to the perturbation can be different each time the simulation is run. To illustrate, we can repeat the simulation 10 times and plot the trajectory of fear repsonse for each.
...
### Visualize Simulation Results for Key Model Components
We can then visualize these maximum fear levels in the two conditions of interest: 
```{r}
plot.new()
...
Finally, we can simulate the impact of treatment. A cognitive behavioral therapy [CBT] intervention is built into simPanic. This treatment includes five intervention components:
...
We can now take a look at the results of this simulated treatment. 
